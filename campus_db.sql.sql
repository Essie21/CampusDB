...
-- Selecting all data from the course table that have 2 credit hours only.

SELECT *
FROM Course
WHERE CrHour = '2'

-- All records from the student table who reside in Lexington

SELECT *
FROM Student
Where City = 'Lexington'

--Records of students who had grade B with their respective courses

SELECT studentNo, StudentGrade, CourseNo
FROM Grade
WHERE StudentGrade = 'B'

-- Records of students from the Takes table who registered for DIS 138

SELECT studentNo, CourseNo, Title
FROM Takes
WHERE CourseNo = 'DIS 138'

-- Records of courses with more than 2 credit hours from the course table

SELECT CourseNo, CrHour
FROM Course
WHERE CrHour > '2'

-- Records of instructors who are not Lujan
...
-- Records of students who are not undergraduates

SELECT LastName, FirstName, Undergrad
FROM Student
WHERE Undergrad = 'No'

-- Records of students who are from Lexington and are also undergraduates

SELECT studentNo, City, Undergrad
FROM Student
WHERE City = 'Lexington' AND Undergrad = 'No'

-- Records of students who had an  or an E from the Grade table
SELECT *
FROM Grade
WHERE StudentGrade = 'A' OR StudentGrade = 'E'



SELECT CourseNo, InstName, CrHour
FROM Course
WHERE InstName > 'Lujan'

-- Another way of doing this using 'OR'

SELECT CourseNo, InstName, CrHour
FROM Course
WHERE CrHour = '3' OR InstName = 'Morris'
